  ...
  return (
    <>
      <div className="flex items-center gap-2">
        ...
        <span className={`text-xs font-medium ${latencyStatus === 'good' ? 'text-emerald-700' : latencyStatus === 'warning' ? 'text-amber-700' : latencyStatus === 'poor' ? 'text-rose-700' : 'text-content-muted'}`}>
          {latencyText}
        </span>
        {latencyStatus === 'good' && (
        ...
        {latencyStatus === 'offline' && (
          <span className="text-xs sm:text-[11px] sm:text-[9px] bg-surface-muted text-content-muted px-1.5 py-0.5 rounded font-medium tracking-wider">Offline</span>
        )}
      </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-indigo-600 font-medium">{onlineUsers.length > 0 ? `${onlineUsers.length} Kolaborator` : 'Aktif'}</span>
      </div>

      <div className="hidden sm:flex items-center gap-4">
        <div className="flex items-center gap-1.5">
          <Globe className="w-3.5 h-3.5 text-content-subtle" />
          <span>Proyek: <strong className="text-content-body">{selectedProjectKey}</strong></span>
        </div>
        <div className="h-3 w-px bg-slate-200" />
        <span className="text-xs sm:text-[11px] sm:text-[9px] text-content-subtle font-medium">v1.6 Live Ready</span>
      </div>
    </>
  );
};
