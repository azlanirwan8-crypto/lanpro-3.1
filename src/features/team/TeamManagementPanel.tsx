...
export const TeamManagementPanel = ({ 
  ...
  const getUserTasks = (person: any) => {
    if (!person) return [];
    const pIds = [person.uid, person.id, person.userId, person.email, person.username, person.displayName].filter(Boolean);
    return tasks.filter(t => {
      ...
      return (
        ...
        (aEmail && person.email && String(aEmail).toLowerCase() === String(person.email).toLowerCase()) ||
        (Array.isArray(assignees) && assignees.some(aId => pIds.some(id => String(id).toLowerCase() === String(aId).toLowerCase())))
      );
    });
  };
  ...
  const assignedTasksCount = tasks.filter(t => t.assigneeId && String(t.type || '').toLowerCase() !== 'epic').length;
  const projectTasksCount = tasks.filter(t => String(t.type || '').toLowerCase() !== 'epic').length;
    ...
    <div className="p-4 md:p-6 w-full h-[calc(100vh-80px)] flex flex-col overflow-hidden animate-in fade-in duration-300 text-left">
      ...
      <div className="shrink-0 space-y-5 pb-1">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-xl font-medium text-slate-800 tracking-tight">Team Management</h1>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Daftar anggota tim yang bergabung dalam proyek {selectedProject ? <span className="font-medium text-slate-700">{selectedProject.name} ({selectedProject.key})</span> : ''}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white p-4 rounded-lg border border-slate-200/80 shadow-2xs flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Active Team</div>
                <div className="text-xl font-medium text-slate-800 mt-0.5">{activeTeamCount}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border border-slate-200/80 shadow-2xs flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Assigned Tasks</div>
                <div className="text-xl font-medium text-slate-800 mt-0.5">{assignedTasksCount}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border border-slate-200/80 shadow-2xs flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Pending Invites</div>
                <div className="text-xl font-medium text-slate-800 mt-0.5">{pendingInvitesCount}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border border-slate-200/80 shadow-2xs flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Project Tasks</div>
                <div className="text-xl font-medium text-slate-800 mt-0.5">{projectTasksCount}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto justify-end">
            <div className="relative min-w-[140px]">
              <select
                ...
                className="w-full pl-3 pr-8 py-1.5 bg-slate-50 border border-slate-200 rounded-md outline-none text-slate-700 font-medium text-xs cursor-pointer appearance-none"
              >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 rounded-md text-xs font-medium shadow-2xs transition-all cursor-pointer"
            >
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 pt-2 text-center flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className="font-medium text-slate-800 text-sm leading-snug truncate group-hover:text-indigo-600 transition-colors">
                        {name}
                      </h3>
                    ...
                    <div className="border-t border-slate-100 my-3 pt-3 grid grid-cols-2 gap-2 text-center">
                      <div className="bg-slate-50 p-2 rounded-md border border-slate-100">
                        <span className="block font-medium text-slate-800 text-sm">{userAssignedTasks.length}</span>
                        <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">Assigned</span>
                      </div>
                      <div className="bg-slate-50 p-2 rounded-md border border-slate-100">
                        <span className="block font-medium text-slate-800 text-sm">{completedTasks.length}</span>
                        <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">Done</span>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-2 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-600 text-slate-700 text-xs font-medium rounded-md transition-colors border border-slate-200/70 shadow-2xs cursor-pointer"
                    >
                      View Profile
                    </button>
                  </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200/80 shadow-2xs overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50/80 border-b border-slate-200/80 text-[11px] font-medium text-slate-500 uppercase tracking-wider">
                  ...
                  {filteredPeople.map((person: any, i) => {
                    ...
                    return (
                      <tr key={person.uid || i} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="px-5 py-3.5 text-center font-medium text-slate-800 text-xs">
                        ...
                        <td className="px-5 py-3.5 text-right">
                          <button
                            onClick={() => setSelectedProfileUser(person)}
                            className="px-3 py-1 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-600 text-slate-700 text-xs font-medium rounded-md transition-colors border border-slate-200/70 shadow-2xs cursor-pointer"
                          >
                            View Profile
                          </button>
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-xl border border-slate-200 shadow-xl max-w-md w-full overflow-hidden text-left">
            ...
            <div className="p-6 pt-0 relative">
              ...
              <h2 className="text-base font-medium text-slate-800">{selectedProfileUser.displayName || 'Anggota Tim'}</h2>
              ...
              {/* Task list preview */}
              <div className="mt-4">
                <h4 className="text-xs font-medium text-slate-700 uppercase tracking-wider mb-2">Tugas yang Ditugaskan</h4>
                <div className="max-h-40 overflow-y-auto space-y-1.5 custom-scrollbar pr-1">
                  {getUserTasks(selectedProfileUser).map(t => (
                    <div key={t.id} className="p-2 bg-white rounded border border-slate-200/80 text-xs flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-medium px-1.5 py-0.5 bg-slate-100 text-slate-600 rounded shrink-0">{t.status}</span>
                    </div>
                  ))}
              ...
              <div className="mt-6 flex justify-end">
                <button
                  onClick={() => setSelectedProfileUser(null)}
                  className="px-4 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium rounded-md transition-colors border border-slate-200 cursor-pointer"
                >
                  Tutup
                </button>
              </div>
