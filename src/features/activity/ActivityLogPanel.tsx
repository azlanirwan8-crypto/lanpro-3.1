    ...
    <div className="flex-1 overflow-auto p-6 md:p-8 bg-slate-50 animate-in fade-in duration-700 custom-scrollbar">
      <div className="space-y-6 w-full">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-200 pb-6 mb-2">
          <div className="flex gap-4 items-center">
            ...
            <div>
              <h1 className="text-3xl font-medium text-slate-800 tracking-tight">System Audit Log</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col justify-between hover:shadow-md transition-shadow h-32 relative overflow-hidden">
              ...
              <div className="text-[10px] font-medium tracking-[0.2em] uppercase text-slate-400 z-10 flex items-center gap-2"><Zap className="w-3.5 h-3.5" /> Total Events</div>
              <div className="text-4xl font-medium text-slate-800 z-10">{activityLogs.length}</div>
          </div>
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col justify-between hover:shadow-md transition-shadow h-32 relative overflow-hidden">
              <div className="absolute right-0 top-0 opacity-[0.03] scale-150 -translate-y-4 translate-x-4"><Users className="w-32 h-32" /></div>
              <div className="text-[10px] font-medium tracking-[0.2em] uppercase text-slate-400 z-10 flex items-center gap-2"><Users className="w-3.5 h-3.5" /> Active Actors (24h)</div>
              <div className="text-4xl font-medium text-slate-800 z-10">{activeActorsCount}</div>
          </div>
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col justify-between hover:shadow-md transition-shadow h-32 relative overflow-hidden">
              <div className="absolute right-0 top-0 opacity-[0.03] scale-150 -translate-y-4 translate-x-4"><Clock className="w-32 h-32" /></div>
              <div className="text-[10px] font-medium tracking-[0.2em] uppercase text-slate-400 z-10 flex items-center gap-2"><Clock className="w-3.5 h-3.5" /> Last Event</div>
              <div className="text-xl font-medium text-slate-800 tracking-tight z-10 flex flex-col">
          ...
          <div className="overflow-x-auto min-h-[500px]">
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead className="bg-white">
                <tr>
                  <th className="px-8 py-5 text-[10px] font-medium text-slate-400 uppercase tracking-[0.2em] border-b border-slate-200 whitespace-nowrap">Timestamp</th>
                  <th className="px-8 py-5 text-[10px] font-medium text-slate-400 uppercase tracking-[0.2em] border-b border-slate-200">Event Signature</th>
                  <th className="px-8 py-5 text-[10px] font-medium text-slate-400 uppercase tracking-[0.2em] border-b border-slate-200">Subject (Actor)</th>
                  <th className="px-8 py-5 text-[10px] font-medium text-slate-400 uppercase tracking-[0.2em] border-b border-slate-200 text-right">Audit Trail ID</th>
                </tr>
              </thead>
                ...
                {filteredLogs.map(log => {
                    ...
                    <tr key={log.id} className="hover:bg-slate-50 border-b border-slate-100 transition-colors group cursor-default">
                      <td className="px-8 py-5 whitespace-nowrap align-top">
                          <div className="text-sm font-medium text-slate-800 tabular-nums tracking-tight">{safeFormat(log.createdAt, 'MMM dd, yyyy')}</div>
                          <div className="text-[10px] font-medium text-slate-400 tabular-nums uppercase mt-1 tracking-widest">{safeFormat(log.createdAt, 'HH:mm:ss.SSS')}</div>
                      </td>
                                ...
                                <>
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap mb-2">
                                      ...
                                      <span className="text-[10px] font-medium text-slate-400 capitalize px-1 ring-1 ring-slate-200 rounded-sm bg-slate-50">
                                        Source: UI
                                      </span>
                                    </div>
                      ...
                      <td className="px-8 py-5 min-w-[200px]">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col min-w-0">
                              <span className="text-[13px] font-medium text-slate-800 truncate">{actor?.displayName}</span>
                              <span className="text-[10px] font-medium text-slate-500 truncate tracking-wide">{actor?.email}</span>
                            </div>
                        </div>
                      </td>
                      <td className="px-8 py-5 text-right w-48 align-middle">
                          <div className="flex flex-col items-end gap-1.5">
                            <div className="font-mono text-[9px] bg-slate-100 text-slate-500 px-2 py-1 rounded border border-slate-200 uppercase font-medium tracking-widest select-all opacity-70 group-hover:opacity-100 transition-opacity" title={log.id}>
