  ...
  return (
    <div className="flex-1 overflow-auto space-y-3.5 pb-16 pr-1 min-h-0 custom-scrollbar">
      {sprints.sort((a,b) => ensureDate(b.startDate).getTime() - ensureDate(a.startDate).getTime()).map(sprint => {
        const sprintTasks = tasks.filter(t => t.sprintId === sprint.id && (t.type || '').toLowerCase() !== 'epic');
        ...
        return (
          <div key={sprint.id} className={cn(
            ...
            sprint.status === 'planned' ? "border-border-subtle/90 hover:border-primary/40" :
            "border-border-subtle/60 bg-surface-sunken/40 opacity-95",
            isExpanded && sprint.status !== 'active' ? "border-primary/30" : ""
          )}>
            <div className="px-4 py-3 flex items-center justify-between gap-3 cursor-pointer hover:bg-surface-sunken/70 transition border-b border-border-faint" onClick={() => setExpandedSprintId(isExpanded ? "" : sprint.id)}>
              <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                <ChevronDown className={cn("w-4 h-4 text-content-subtle transition-transform duration-200 shrink-0", isExpanded ? "rotate-0" : "-rotate-90")} />
                <div className="flex items-center gap-2.5 min-w-0 flex-1">
                  ...
                  <span className={cn("text-xs sm:text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border shrink-0 whitespace-nowrap", 
                    ...
                    sprint.status === 'planned' ? "bg-surface-muted text-content-secondary border-border-subtle/60" : "bg-blue-50 text-blue-700 border-blue-100"
                  )}>
                    {sprint.status === 'active' ? (isOverdue ? 'OVERDUE' : 'ACTIVE') : sprint.status === 'planned' ? 'PLANNED' : 'COMPLETED'}
                  </span>
                  <div className={cn("hidden md:flex items-center gap-1 text-xs font-medium shrink-0 whitespace-nowrap ml-0.5", isOverdue ? "text-red-500 font-medium" : "text-content-muted")}>
                    <Calendar className={cn("w-3.5 h-3.5 shrink-0", isOverdue ? "text-red-500" : "text-content-subtle")} />
                    <span>
              ...
              <div className="flex items-center gap-4 shrink-0">
                ...
                <div className="hidden lg:flex items-center gap-2.5 w-44" onClick={(e) => e.stopPropagation()}>
                  ...
                  <span className="text-xs sm:text-[11px] font-medium text-content-secondary shrink-0 w-9 text-right">{completionPercentage}%</span>
                </div>

                <div className="flex items-center gap-3">
                  <div className="text-right hidden md:block">
                    <span className="text-xs sm:text-[10px] font-medium text-content-subtle block uppercase leading-none">Issues</span>
                    <span className="text-xs font-medium text-content-body">{sprintTasks.length}</span>
                  </div>
                  <div className="text-right hidden md:block">
                    <span className="text-xs sm:text-[10px] font-medium text-content-subtle block uppercase leading-none">Points</span>
                    ...
                    <div className="flex items-center gap-1.5 pl-2 border-l border-border-subtle/80">
                      ...
                      {sprint.status === 'active' && (
                        <button 
                          ...
                          className="h-7 px-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-medium rounded-md transition-all flex items-center gap-1 shadow-2xs shrink-0 cursor-pointer"
                        >
                          <CheckCircle2 className="w-3 h-3" /> <span>COMPLETE</span>
                        </button>
                      )}
                      <button onClick={(e) => { e.stopPropagation(); setEditingSprint(sprint); setIsEditSprintModalOpen(true); }} className="p-1.5 text-content-subtle hover:text-primary hover:bg-surface-muted rounded-md transition-colors cursor-pointer">
                        <Edit2 className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); handleDeleteSprint(sprint.id); }} className="p-1.5 text-content-subtle hover:text-red-600 hover:bg-red-50 rounded-md transition-colors cursor-pointer">
                          ...
                          return (
                            <div 
                              ...
                              className={cn(
                                ...
                                sprintTasks.length === 0 && !snapshot.isDraggingOver && "border-border-subtle"
                              )}
                            >
                              {sprintTasks.length === 0 && !snapshot.isDraggingOver ? (
                                <div className="flex items-center justify-center p-6 text-center">
                                  <p className="text-xs font-medium text-content-subtle">Belum ada task di sprint ini. Tarik task dari Backlog di sebelah kiri ke sini.</p>
                                </div>
                              ) : (
                                <>
                                  {sprintEpics.flatMap(epic => {
                                    ...
                                    return [
                                      <div key={`sprint-header-${epic.id}`} className="flex items-center justify-between px-3 py-1.5 bg-purple-50/50 border border-purple-100 rounded-md mb-2 mt-2 first:mt-0">
                                        ...
                                        <div className="text-xs sm:text-[10px] font-medium text-purple-600 bg-surface border border-purple-100 px-2 py-0.2 rounded-md">{items.length}</div>
                                      </div>,
                                      ...items.map((task) => {
                                  ...
                                  {(() => {
                                    ...
                                    if (items.length === 0) return [];
                                    return [
                                      <div key="sprint-header-standalone" className="flex items-center justify-between px-3 py-1.5 bg-surface-muted/70 border border-border-subtle/60 rounded-md mb-2 mt-2 first:mt-0">
                                        <div className="flex items-center gap-2">
                                          <Target className="w-3.5 h-3.5 text-content-muted" />
                                          <span className="text-xs sm:text-[10px] font-medium text-content-secondary uppercase tracking-wider leading-none">Standalone Tasks</span>
                                        </div>
                                        <div className="text-xs sm:text-[10px] font-medium text-content-secondary bg-surface border border-border-subtle/60 px-2 py-0.2 rounded-md">{items.length}</div>
                                      </div>,
                                      ...items.map((task) => {
                                        const dndIndex = _draggablesRenderedCount++;
                                        return (
