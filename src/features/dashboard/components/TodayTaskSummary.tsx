...
export const TodayTaskSummary: React.FC<TodayTaskSummaryProps> = ({ statusData }) => {
  return (
    <div className="bg-surface rounded-xl p-5 border border-border-faint/80 shadow-soft flex flex-col h-full">
      <h3 className="text-sm font-medium text-content-strong mb-4">Today's Task Summary</h3>
      
      {statusData.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center text-content-subtle py-6">
              ...
              <div key={index} className="space-y-1">
                <div className="flex justify-between items-center text-xs">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-content-body">{status.name}</span>
                  </div>
                  <span className="font-medium text-content">{status.current_count} / {status.total_count}</span>
                </div>
      ...
      {statusData.length > 0 && (
        <div className="pt-4 mt-2 border-t border-border-faint">
        </div>
      )}
    </div>
  );
