...
export const IssueListView: React.FC<IssueListViewProps> = (props) => {
  ...
  const canEditIssue = (issue: Task): boolean => {
    ...
    const hasRole = hasPermission(userRole, 'list', 'update', isReporter, currentUserProfile?.permissions);
  ...
  const canDeleteIssue = (issue: Task): boolean => {
    if (!issue) return false;
    const isReporter = isUserReporter(issue);
    const hasRole = hasPermission(userRole, 'list', 'delete', isReporter, currentUserProfile?.permissions);
    const isLeadOrAdmin = ['admin', 'manager', 'head'].includes(userRole || '');
    return hasRole || isReporter || isLeadOrAdmin;
  };
          ...
          {canReorder && (
             <td className="w-8 px-1 text-center border-r border-border-faint/50 bg-surface">
                {depth === 0 && dragHandleProps ? (
                   <div {...dragHandleProps} className="cursor-grab active:cursor-grabbing text-content-subtle hover:text-content-secondary flex justify-center py-1 outline-none">
  ...
  return (
    <div className="flex-1 p-2 sm:p-4 md:p-6 bg-[#f8fafc] overflow-hidden flex flex-col w-full h-full">
